...
- Alibaba provider ที่ตั้งให้ใช้ `@ai-sdk/openai` เพื่อส่ง request ผ่าน Responses API
...
ควรใช้ model block แบบเต็มตามตัวอย่าง เพราะ custom provider model อาจไม่แสดงอย่างถูกต้องเมื่อไม่มีข้อมูล capabilities, limit และ modalities การใช้ `@ai-sdk/openai` ระดับ provider จะทำให้โมเดลที่กำหนดไว้เรียก `/responses` และเป็นวิธีที่ง่ายที่สุดเมื่อ provider นี้มีเฉพาะ Qwen Responses API models

หาก provider เดียวกันมี GLM, DeepSeek หรือโมเดลอื่นที่ต้องใช้ Chat Completions ให้คง `@ai-sdk/openai-compatible` ระดับ provider และเพิ่ม `"provider": { "npm": "@ai-sdk/openai" }` เฉพาะภายใน Qwen model block
...
- ตรวจว่า provider ใช้ `@ai-sdk/openai` หากเป็น mixed provider ให้ตรวจว่า Qwen model มี model-level override เป็น `@ai-sdk/openai`
